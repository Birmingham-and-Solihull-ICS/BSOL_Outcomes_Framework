...
# age_min: (Optional) To accept indicator with specific min age & ALL ages
# age_max: (Optional) To accept indicator with specific max & ALL ages
...
get_numerator <- function(indicator_data, indicator_id, reference_id = NULL, age_min = NULL, age_max = NULL) {
  ...
  output <- output %>%
    ...
    group_by(Ethnicity_Code, Fiscal_Year, LAD22CD,
             WD22CD, WD22NM, Locality) %>%  
    summarise(Numerator = sum(Numerator, na.rm = TRUE), .groups = 'drop') %>% 
    select(Numerator, Ethnicity_Code, Fiscal_Year, LAD22CD,
           WD22CD, WD22NM, Locality) %>% 
...
get_denominator <- function(age_min = NULL, age_max = NULL, pop_estimates, numerator_data){
  ...
  if (!is.null(age_min) & !is.null(age_max)) {
    ...
    pop_estimates <- pop_estimates %>%
      filter(age_86_categories_code <= age_max)
  }
...
# Call the function 
denominator_data <- get_denominator(age_min = age_min,  
...
calculate_crude_rate <- function(indicator_id, denominator_data, numerator_data, aggID, genderGrp, ageGrp, multiplier = 100000) {
  ...
  calculate_rate <- function(id, group_vars) {
    ...
    if (id == "WD22NM") {
      ...
      joined_data <- joined_data %>%
        group_by(across(all_of(group_vars))) # No need for left joining for BSOL
    }
    
    # Summarize and aggregate data to calculate rate
  ...
  output <- final_results %>%
    ...
    select(
      ...
      DataQualityID, IndicatorStartDate, IndicatorEndDate
    )
...
write.csv(crude_rate_result, "output.csv")
...
calculate_aggregated_rate <- function(crude_rate_data, aggregated_years = c(3, 5)) {
  ...
  for (year in aggregated_years) {
    aggregated_data <- crude_rate_data %>%
      mutate(FiscalYearStart = as.numeric(substr(FiscalYear, 1, 4))) %>%
      group_by(PeriodStart = (FiscalYearStart %/% year) * year,
    ...
    aggregated_output <- aggregated_data %>%
      group_by(AggregationType, AggregationLabel, Gender, AgeGroup, IMD, EthnicityCode, 
      ...
      rename(IndicatorValue = value, LowerCI95 = lowercl, UpperCI95 = uppercl) %>%
      ungroup() %>%
      mutate(
        ValueID = row_number(),
