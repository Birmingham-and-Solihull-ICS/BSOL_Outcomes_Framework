...
,			HCCSReconciliationPoint							AS [TimePeriod]		--What field is this derived from?
...
LEFT JOIN	[Reference].[dbo].[DIM_tbDate] T2
ON			CONVERT(date, S1FirstArrivalDateTime) = T2.Date


WHERE		LA_District_of_Residence IN ('E08000025','E08000029')
--			[CCG_of_Residence] IN ('13P','04X','05P','15E','QHL')		--BSOL Resident patient
AND			[S2THROMBOLYSIS] ='Y'										--patients who were given thrombolysis


GROUP BY	[S1ETHNICITY]										
,			HCCSReconciliationPoint
,			[CCG_of_Residence]
,			[LSOA_OF_RESIDENCE]
,			[PseudoNHSNo]
...
WHERE		LA_District_of_Residence IN ('E08000025','E08000029')
--[CCG_of_Residence] IN ('13P','04X','05P','15E','QHL')
...
--SELECT	* FROM #CV8_Numerator_Grouped
--SELECT	* FROM #CV8_Denominator_Grouped ORDER BY Numerator DESC
